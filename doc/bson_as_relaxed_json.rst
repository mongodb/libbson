...
The :symbol:`bson_as_relaxed_json()` encodes ``bson`` as a UTF-8 string in the relaxed `MongoDB Extended JSON format`_.
...
.. code-block:: c
  ...
  char *str = bson_as_relaxed_json (doc, NULL);
  printf ("%s\n", str);
  bson_free (str);
